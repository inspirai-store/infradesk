...
// 注意：不要直接用 v-model:value 绑定 activeId。
// 因为连接列表（options）尚未加载时，NSelect 会把 raw value（例如 2/4）直接渲染出来。
// 这里用 displayValue 做“展示层”的值：仅当 options 中确实存在该 id 时才显示，否则显示 placeholder。
const displayValue = computed<number | null>(() => {
  const id = activeId.value
  if (id == null) return null
  return options.value.some(o => o.value === id) ? id : null
})

function onUpdateValue(val: number | null) {
  activeId.value = val
}
