...
async function handleImport() {
  ...
  try {
    // 第一次尝试导入（不强制覆盖）
    const response = await k8sApi.importConnections(toImport, false)
    ...
    if (hasSkipped) {
      ...
      dialog.warning({
        ...
        onPositiveClick: async () => {
          ...
          importing.value = true
          try {
            const overrideResponse = await k8sApi.importConnections(toImport, true)
...
function toggleService(service: DiscoveredService) {
  const key = getServiceKey(service)
  if (selectedServices.value.has(key)) {
    selectedServices.value.delete(key)
  } else {
    selectedServices.value.add(key)
  }
}
...
const columns: DataTableColumns<DiscoveredService> = [
  {
    type: 'selection',
    disabled: (row) => !row.has_credentials,
  },
  {
    title: '服务名',
    key: 'name',
  ...
  >
    <div class="discovery-container">
      ...
      <div class="kubeconfig-section" :style="{ opacity: discovering ? 0.6 : 1, pointerEvents: discovering ? 'none' : 'auto' }">
        <NAlert 
          v-if="!kubeconfigUploaded" 
          type="info" 
          title="本地测试模式"
          closable
        >
          在本地测试时，可以上传 kubeconfig 文件来选择要扫描的集群
        </NAlert>
      ...
      <NSpin :show="discovering">
        ...
        <div v-if="hasServices" class="services-table">
          <NDataTable
            ...
            @update:checked-row-keys="(keys: string[]) => selectedServices = new Set(keys)"
            :pagination="false"
            :max-height="400"
            size="small"
          />
